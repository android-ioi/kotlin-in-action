...
![backing field](images/backing_field.png)
...
when은 자바의 switch와는 달리, 임의의 객체를 허용한다.
```kt
...
위의 코드에서 when은 객체의 동등성(equality)을 이용하여 매치한다. mix(RED, YELLOW)와 mix(YELLOW, RED) 모두 set의 성질에 따라 동등하게 처리된다.
(다만, 위의 코드는 분기 조건마다 set을 생성하므로 비효율적이다.)

kotlin 1.3부터는 when의 대상에 변수를 대입할 수 있다.
...
인자가 없는 when을 사용할 경우, 각 분기의 조건마다 boolean을 계산하는 식이 있어야 한다.
```kt
when {
    code == 0 -> "Success"
    code == -1 -> "Error"
...
```kt
fun isLetter(c: Char) = c in 'a'..'z' || c in 'A'..'Z' // 'a' <= c && c <= 'z'의 형태로 변환된다.
...
```
컬렉션에서도 in 연산을 사용할 수 있다. (contains)
```kt
...
예외 처리는 기본적으로 try - catch, finally로 처리하고, 메소드 옆에 throws절이 없다.
try는 `식(Expression)`이며, 자바는 Checked Exception 이라는 반드시 예외 처리를 해줘야 하는 경우가 있는가 하면, 코틀린은 Checked exception이 없다.(https://kotlinlang.org/docs/exceptions.html#checked-exceptions)